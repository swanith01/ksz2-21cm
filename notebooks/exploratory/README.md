...
|---|---|---|
| `wedge_diagnostic_kSZ2x21cm_unsquared.py` | diag1 (foreground filter regions) + CELL 9 (wedge destroys the unsquared statistic) — the key diagnostic explaining why the pipeline uses kSZ²×21cm² (squared) as the paper's main result instead of kSZ²×21cm (unsquared) | Extracted and archived, not part of the reproducible pipeline. Two confirmed unit bugs from the original code are flagged prominently in its header (CELL 9's mislabeled y-axis; a three-way inconsistency in the k→ℓ conversion across different cells) — read the header before trusting any absolute numbers from it. |
| `original_monolith_kSZ_Squared_21cm_11Jun_CLUSTER.py` | Your original full pipeline script, unchanged | Reference copy. Most cells have been split into `scripts/` + `src/` (see the header of this file for the exact mapping). CELLs 7c (partially — diag1 now extracted above), 8a, 8b, 8.5, 8c (diagnostic plots) have **not** been individually extracted yet — they still only exist here. |
